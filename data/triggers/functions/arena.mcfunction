...
execute as @a[scores={arena=1..},team=!Blue,team=!Red] run tellraw @s {"text": "Select a team before going to the arena"}
execute as @a[scores={arena=1..},team=!Blue,team=!Red] run scoreboard players set @s arena 0
    # a kit has been chosen
execute as @a[scores={arena=1..},tag=!archer,tag=!bomber,tag=!scout,tag=!tank,tag=!wizard,tag=!sniper] run tellraw @s {"text": "Select a kit before going to the arena"}
